...
import 'package:fake_store_app/utils/constants.dart';
...
import '../../../components/frosted_dialog.dart';
...
class LoginPageState extends State<LoginnPage> with TickerProviderStateMixin {
  ...
  List<Widget> _signUp() {
    return [
          ...
          try {
            ...
            final persistSessionString =
                await RepositoryProvider.of<Repository>(context).signUp(
                    email: _emailController.text,
                    password: _passwordController.text);
                    // kys becuase it app.dart spot not made which consists multi repositoryprovider parent class
            ...
            context.showSnackbar(err.message ?? 'Error kys signing up');
          } catch (err) {
            setState(() {
              _isLoading = false;
            });
            context.showErrorSnackbar('Errorzzsigning up');
            print(err);
          }
        },
      ),
      const SizedBox(height: 24.5),
      const Text(
        'By signing in, you agree to the Terms of Service and Privacy Policy dialog or blog ',
      ),
    ];
  }
